			...
			else
			{
				...
				else
				{
					...
					if($organizador)
					{
						...
						$bdy = "Se registró un evento nuevo en la web: \"".$evento->nombre."\" en la disciplina: ".Codes::getEventoNombre($evento->disciplina).". Creado por: \"".$organizador->nombre." ".$organizador->apellido."\" (".$organizador->correo.")<br/><br/>Información del evento ".$evento->nombre."<br/><br/><b>Fecha: </b>".$evento->fechaStr."<br/><b>Web del evento: </b> <a href='".$evento->urlEvento."'>".$evento->urlEvento."</a><br/><br/>Para mayor información: <br/><b>Organizador:</b> ".$organizador->nombre." ".$organizador->apellido."<br/><b>Correo:</b> <a href='mailto:".$organizador->correo."'>".$organizador->correo."</a><br/><b>Teléfono:</b>".$organizador->telefono_1."<br/><br/><a href=\"".DIR."evento/".$evento->id."\">Ver evento</a>";
...
/*
	...
	public function modEventoInfoPOST($eventoId)
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$disciplina 		= filter_input(INPUT_POST, 'disciplina');
			$eqCantIntegrantes 	= filter_input(INPUT_POST, 'eqCantIntegrantes');
			$eqMismoGenero 		= 0;
			$permiteRegistros 	= 0;
			$visible 			= 0;
			$wodsvisible 		= 0;
			...
			if($removerLogo)
			{
				if($evento->archivoFoto)
				{
					if(file_exists("./".EVENTPICTUREPATH.$evento->archivoFoto))
			...
			else
			{
				...
				if($archivo['error'] == UPLOAD_ERR_OK)
				{
					...
					if($ext)
					{
						...
						$proc = move_uploaded_file($_FILES['archivoFoto']['tmp_name'], "./".EVENTPICTUREPATH.$nombreArchivo.".".$ext);

						if($proc)
						{
							$eventos->actualizarEventoInfo(array('archivoFoto' => $nombreArchivo.".".$ext), array('id' => $evento->id));
			...
			else
			{
				...
				$new['eqCantIntegrantes'] 	= $eqCantIntegrantes;
				$new['eqMismoGenero'] 		= $eqMismoGenero;
			...
			foreach ($toUpdate as $key => $value)
			{
				if($_FILES['logo']['error'][$key] == UPLOAD_ERR_OK)
				{
					...
					if($ext)
					{
						...
						if($patrocinadores[$key]->archivoFoto)
						{
							if(file_exists("./".SPONSORPICTUREPATH.$patrocinadores[$key]->archivoFoto))
								unlink("./".SPONSORPICTUREPATH.$patrocinadores[$key]->archivoFoto);
							
							$eventos->actualizarPatrocinador($key, array('archivoFoto' => null));
						}
						...
						if($proc)
						{
							$eventos->actualizarPatrocinador($key, array('archivoFoto' => $nombreArchivo.".".$ext));
			...
			foreach ($toInsert as $key => $value)
			{
				...
				if($_FILES['logoN']['error'][$key] == UPLOAD_ERR_OK)
				{
					...
					if($ext)
					{
						...
						if($proc)
						{
							$eventos->actualizarPatrocinador($patId, array('archivoFoto' => $nombreArchivo.".".$ext));
	...
	public function modEventoAtletasAprobar($eventoId, $info = array())
	{
		...
		if(!$evento->registroTeam)
		{
			...
			$registrados = $eventos->getAtletasRegistrados($evento->id);
			$pagos = $eventos->getPagos($pendientes);
		}
		else
		{
			$pendientes = $eventos->getEquiposPendientes($evento->id);
			$registrados = $eventos->getEquiposRegistrados($evento->id);
		...
		$data['atletasRegistrados'] = $registrados;
		$data['pagos'] = $pagos;
		$data['logeado'] = $logeado;
	...
	public function modEventoAtletasAprobarPOST($eventoId)
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$cantNuevos = count($aceptar) - count($rechazar);
				...
				foreach ($aceptar as $key => $value)
				{
					...
					if($resultado > 0)
					{
						...
						if(!empty($persona->correo))
						{
							...
							$bdy = "Saludos ".$persona->nombre." ".$persona->apellido.", le informamos que su solicitud de registro en el evento \"".$evento->nombre."\" ha sido aprobado.<br/><br/>Información del evento: ".$evento->nombre."<br/><b>Fecha: </b>".$evento->fechaStr."<br/><b>Categoría: </b>".$categoria->nombre."<br/><b>Web del evento: </b> <a href='".$evento->urlEvento."'>".$evento->urlEvento."</a><br/><br/>Para mayor información: <br/><b>Organizador:</b> ".$organizador->nombre." ".$organizador->apellido."<br/><b>Correo:</b> <a href='mailto:".$organizador->correo."'>".$organizador->correo."</a><br/><b>Teléfono:</b>".$organizador->telefono_1;
							$strMail = str_replace("[MENSAJE]", $bdy, $txt);
							$strMail = str_replace("[LOGO1]", DIR."app/templates/".TEMPLATE."/img/logoCuadrado.png", $strMail);
			...
			foreach ($rechazar as $key => $value)
			{
				...
				if($resultado > 0)
				{
					...
					if(!empty($persona->correo) && $persona->correo_verificado)
					{
						$gestor = fopen("app/templates/mail/mail.html", "r");
						$txt = "";
						...
						$bdy = "Saludos ".$persona->nombre." ".$persona->apellido.", <br/><br/>Le informamos que su solicitud de registro en el evento \"".$evento->nombre."\" ha sido rechazado.<br/><br/>Información del evento: ".$evento->nombre."<br/><b>Fecha: </b>".$evento->fechaStr."<br/><b>Categoria: </b>".$categoria->nombre."<br/><b>Web del evento: </b> <a href='".$evento->urlEvento."'>".$evento->urlEvento."</a><br/><br/>Para mayor información: <br/><b>Organizador:</b> ".$organizador->nombre." ".$organizador->apellido."<br/><b>Correo:</b> <a href='mailto:".$organizador->correo."'>".$organizador->correo."</a><br/><b>Teléfono:</b>".$organizador->telefono_1;
						$strMail = str_replace("[MENSAJE]", $bdy, $txt);
						$strMail = str_replace("[LOGO1]", DIR."app/templates/".TEMPLATE."/img/logoCuadrado.png", $strMail);
						...
						$mail->setFrom(SITEEMAIL);
						$mail->addAddress($persona->correo);
						$mail->subject(SITETITLE." - Registro en evento rechazado: ".$evento->nombre);
						$mail->body($strMail);
						$mail->send();
					}
				}
	...
	public function modEventoAtletasAprobadosPOST($eventoId)
	{
		...
		if(isset($_POST['submit']))
		{
			...
			{
				foreach ($_POST['registro'] as $registroId => $categoriaId)
				{
					if($atletas[$registroId]->categoriaId != $categoriaId)
						$registros[$registroId] = $categoriaId;
				}
			}

			if(isset($_POST['borrar']))
			{
				foreach ($_POST['borrar'] as $registroId => $categoriaId)
				{
					$borrar[$registroId] = $categoriaId;

					if(isset($registros[$registroId]))
						unset($registros[$registroId]);
				}
			}
...
*/
	...
	public function modEventoMiResultado($eventoId, $categoriaId = 0, $wodId = 0, $info = array())
	{
		...
		foreach ($resultadosViejos as $keyRA => $value)
		{
			foreach ($value as $keyWID => $value2)
			{
				foreach ($value2 as $keyATT => $valueRes)
				{
					$resultados[$valueRes->id] = $valueRes;
				}
			}
		}
	...
	public function aprobarEvento($eventoId)
	{
		...
		if($aprobacion > 0)
		{
			...
			if($organizador->correo_verificado)
			{
				...
				$bdy = "Saludos ".$organizador->nombre." ".$organizador->apellido.", <br/><br/>Le informamos que su evento de nombre \"".$evento->nombre."\" ha sido aprobado correctamente.<br/><br/>A partir de ahora su evento podrá ser mostrado en la lista de eventos del sitio.";
				$strMail = str_replace("[MENSAJE]", $bdy, $txt);
				$strMail = str_replace("[LOGO1]", DIR."app/templates/".TEMPLATE."/img/logoCuadrado.png", $strMail);
				...
				$mail->subject(SITETITLE." - Evento Aprobado (".$evento->nombre.")");
				$mail->body($strMail);
				$mail->send();
			}
		}
	...
	{		
		...
		if($pago)
		{
			...
			$equipo = $equipos->get($solicitud->equipoId);
	...
	public function modEventoEquipoCrearPOST($eventoId)
	{
		...
		if(isset($_POST['submit']))
		{
			...
			else
			{
				...
				Url::redirect('cuenta/equipo/mod/'.$equipoId.'/agregarintegrantes');
			}
		}
	}
	...
	public function modEventoEquipoCrearParticipantesPOST($eventoId, $equipoId, $info = array())
	{
		...
		if(isset($_POST['submit']))
		{
			...
			{
				$equipos->agregarAtleta($equipoId, $key);
